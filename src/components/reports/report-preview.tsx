...
import type { ReportData, SoilIndicator, CropRecommendation, TreatmentRecommendation, SeasonalActivity, ComprehensiveReport, CropReport } from "@/types"
...
// Add type guards
const isComprehensiveReport = (report: ReportData): report is ComprehensiveReport => {
  return 'treatmentRecommendations' in report && 'seasonalPlan' in report;
}

const isCropReport = (report: ReportData): report is CropReport => {
  return 'cropRecommendations' in report && !('treatmentRecommendations' in report);
}
...
export function ReportPreview({ reportData, includeTreatment, includeSeasonalPlan }: ReportPreviewProps) {
  ...
  const getRatingIcon = (rating: string | undefined) => {
    if (!rating) return <Info className="h-5 w-5 text-gray-500" />;
    switch (rating.toLowerCase()) {
      case "excellent":
      case "good":
  ...
  const getRatingProgress = (rating: string | undefined) => {
    if (!rating) return 0;
    switch (rating.toLowerCase()) {
      case "excellent":
        return 95
  ...
  return (
    <div className="space-y-8 print:text-black">
      ...
      {(isComprehensiveReport(reportData) || isCropReport(reportData)) && reportData.cropRecommendations.length > 0 && (
        <section>
          <h2 className="text-2xl font-bold mb-4 text-green-700 border-b pb-2">Crop Recommendations</h2>
          <div className="space-y-4">
            {reportData.cropRecommendations.map((crop: CropRecommendation, index: number) => (
              <Card key={index}>
      ...
      {includeTreatment && isComprehensiveReport(reportData) && reportData.treatmentRecommendations.length > 0 && (
        <section>
          <h2 className="text-2xl font-bold mb-4 text-green-700 border-b pb-2">Treatment Recommendations</h2>
          <div className="space-y-4">
            {reportData.treatmentRecommendations.map((treatment: TreatmentRecommendation, index: number) => (
              <Card key={index}>
                <CardContent className="p-6">
      ...
      {includeSeasonalPlan && isComprehensiveReport(reportData) && reportData.seasonalPlan.length > 0 && (
        <section>
          <h2 className="text-2xl font-bold mb-4 text-green-700 border-b pb-2">Seasonal Management Plan</h2>
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {reportData.seasonalPlan.map((season: SeasonalActivity, index: number) => (
                <Card key={index}>
                  <CardHeader className="pb-2">
